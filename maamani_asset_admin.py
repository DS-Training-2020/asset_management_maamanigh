...
def init_db():
    ...
    exists = c.fetchone()["n"]
    if exists == 0:
        # Use Streamlit secret ADMIN_PASSWORD if available, else fall back to 'admin123'
        try:
            admin_pw = st.secrets.get("ADMIN_PASSWORD", None)
        except Exception:
            admin_pw = None
        admin_pw = admin_pw or "admin123"
        add_user("admin", admin_pw, role="admin", _conn=conn)
...
def delete_asset(asset_tag: str):
    conn = get_conn()
    c = conn.cursor()
    c.execute("DELETE FROM assets WHERE asset_tag=?", (asset_tag,))
    conn.commit()
    conn.close()

def delete_user(username: str):
    # Safeguard: never delete the built-in admin via SQL
    conn = get_conn()
    c = conn.cursor()
...
if not st.session_state.auth["logged_in"]:
    ...
    if menu == "User Management":
        if not is_admin:
            ...
            st.markdown("### 🔴 Delete User (Admin only)")
            if not users_df.empty:
                # Exclude built-in admin from selection to reduce mistakes
                selectable = [u for u in users_df["username"].tolist() if u != "admin"]
                if not selectable:
                    st.info("No deletable users available.")
                else:
                    del_user = st.selectbox("Select User to Delete", selectable, key="del_user_select")
                    confirm_text = st.text_input("Type DELETE to confirm", key="del_user_confirm")
                    if st.button("Delete User"):
                        if del_user == st.session_state.auth["username"]:
                            st.warning("You cannot delete the account you are currently logged in with.")
    ...
    # ---------------- VIEW ASSETS ----------------
        ...
        if is_admin and not df.empty:
            st.markdown("### 🔴 Delete Asset (Admin only)")
            del_asset = st.selectbox("Select Asset Tag to Delete", df["asset_tag"].tolist(), key="del_asset_select")
            confirm_text = st.text_input("Type DELETE to confirm", key="del_asset_confirm")
            if st.button("Delete Asset"):
                if confirm_text.strip().upper() != "DELETE":
                    st.warning("Please type DELETE to confirm.")
                else:
                    delete_asset(del_asset)
                    st.success(f"Asset '{del_asset}' deleted.")
                    st.rerun()
    ...
    # ---------------- ADD / UPDATE ----------------
        ...
        with st.form("asset_form", clear_on_submit=False):
            ...
            if existing.get("disposal_date"):
                form["Disposal Date"] = st.date_input(
                    "Disposal Date", value=pd.to_datetime(existing["disposal_date"]).date()
                )
            else:
                add_disposal = st.checkbox("Add Disposal Date?", value=False, key=f"chk_disposal_{selected_tag or 'NEW'}")
                if add_disposal:
                    form["Disposal Date"] = st.date_input(
                        "Disposal Date", value=date.today(), key=f"date_disposal_{selected_tag or 'NEW'}"
                    )
                else:
        ...
        if submitted:
            ...
            if action == "Add New Asset":
                ...
                if get_asset(new_tag):
                    st.error(
                        f"Asset Tag '{new_tag}' already exists. Change Serial/Dept/Location/Name to generate a different tag."
                    )
                else:
